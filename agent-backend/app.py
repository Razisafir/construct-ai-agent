...
# LSP and Completions service instances
_lsp_manager = None
_completion_service = None


def _get_lsp_manager():
    """Lazily initialise and return the LSP manager."""
    global _lsp_manager
...
class InlineCompletionRequest(BaseModel):
    ...
    language: str = Field(..., description="Programming language ID")
    file_path: str = Field("", description="Path of the file being edited")
    line: int = Field(0, description="Current line number (1-based)")
    column: int = Field(0, description="Current column number (1-based)")


class CompletionAcceptRequest(BaseModel):
    completion_id: str = Field(..., description="ID of the completion")
    accepted: bool = Field(True, description="Whether the completion was accepted")
...
async def lsp_status() -> dict:
    """Get the status of all LSP servers (installed, running)."""
    manager = _get_lsp_manager()
    return {"servers": manager.get_status()}


@app.post("/lsp/start")
async def lsp_start(req: LSPStartRequest) -> dict:
    """Start an LSP server for a language.

    The server will be spawned as a subprocess and initialized
    with the given project path as rootUri.  If already running,
    returns success immediately.
    """
    manager = _get_lsp_manager()

    # Check if installed first
    if not manager.is_installed(req.language):
        hint = manager.get_install_hint(req.language)
        return {
            "started": False,
            "error": f"LSP server for '{req.language}' not installed",
            "install_hint": hint,
        }

    started = await manager.start(req.language, req.project_path)
    return {
        "started": started,
...
async def lsp_request(req: LSPRequestModel) -> dict:
    """Send an LSP request to a running language server.

    Used by the frontend to forward LSP requests (hover, goto-definition,
    completion, etc.) through the backend to the language server process.
    """
    manager = _get_lsp_manager()
    response = await manager.request(req.language, req.method, req.params)
    if response is None:
        return {"error": f"No running LSP server for '{req.language}'"}
    return response


@app.post("/lsp/notify")
async def lsp_notify(req: LSPRequestModel) -> dict:
    """Send an LSP notification to a running language server.

    Notifications do not expect a response (e.g. textDocument/didOpen,
    textDocument/didChange, textDocument/didClose).
    """
    manager = _get_lsp_manager()
    await manager.notify(req.language, req.method, req.params)
    return {"sent": True}


@app.websocket("/lsp/{language}")
async def lsp_websocket(websocket: WebSocket, language: str):
    """WebSocket bridge between the Monaco frontend and an LSP server.

    The frontend connects to this WebSocket and sends LSP requests as
    JSON messages.  The backend forwards them to the appropriate language
    server process and returns the response.  This enables real-time
    communication for features like hover, completion, and diagnostics.
    ...
    from core.lsp_manager import LSPManager
    server_key = manager._get_server_for_language(language)
    if not server_key:
        await websocket.send_json({"error": f"No LSP server configured for '{language}'"})
        await websocket.close()
        return

    # Auto-start if not running
    if server_key not in manager.connections or not manager.connections[server_key].initialized:
        project_path = os.path.expanduser("~/construct-projects/default")
        os.makedirs(project_path, exist_ok=True)
        started = await manager.start(language, project_path)
        if not started:
            hint = manager.get_install_hint(language)
            await websocket.send_json({
                "error": f"LSP for '{language}' not available",
                "install_hint": hint,
            })
            await websocket.close()
            return
    ...
    try:
        while True:
            # Receive LSP message from frontend
            ...
            response = await manager.request(language, method, params)

            # Send back to frontend
            if response:
                await websocket.send_json(response)
            else:
                await websocket.send_json({"error": f"LSP request failed for {method}"})

    except WebSocketDisconnect:
        logger.debug("LSP WebSocket disconnected for '%s'", language)
    except Exception as exc:
        logger.error("LSP WebSocket error for '%s': %s", language, exc)
        try:
            await websocket.close()
        except Exception:
            pass
...
# ===========================================================================
# AI Inline Completions Endpoints
# ===========================================================================

@app.post("/completions/inline")
async def inline_completion(req: InlineCompletionRequest) -> dict:
    """Get an AI inline code completion (ghost text).

    The frontend's InlineCompletionsProvider calls this endpoint
    with the code context around the cursor position.  The backend
    tries multiple LLM providers (Groq, Ollama, OpenAI) to generate
    a short, contextually relevant completion.

    Returns the completion text, or null if no provider is available.
    """
    service = _get_completion_service()

    if not service.is_available:
        return {"completion": None, "error": "No completion provider configured"}
    ...
    try:
        ...
        logger.error("Inline completion failed: %s", exc)
        return {"completion": None, "error": str(exc)}


@app.post("/completions/track")
async def track_completion(req: CompletionAcceptRequest) -> dict:
    """Track whether a completion was accepted or dismissed.

    Used for analytics and model improvement.  The frontend calls
    this when the user accepts (Tab) or dismisses (Escape) a completion.
    """
    service = _get_completion_service()
    service.track_acceptance(req.completion_id, req.accepted)
    return {"tracked": True}


@app.get("/completions/stats")
async def completion_stats() -> dict:
    """Get completion service statistics and provider status."""
    service = _get_completion_service()
...
async def list_mcp_servers() -> dict:
    ...
    Returns information about each connected server including
    available tools, health status, and latency.
    """
    try:
        if _tool_registry is None:
        ...
        for name, conn in manager._connections.items():
            servers.append({
                ...
                "last_health_check": str(conn.last_health_check) if conn.last_health_check else None,
            })
        return {"servers": servers, "total": len(servers)}
    except Exception as exc:
        logger.error("Failed to list MCP servers: %s", exc)
        return {"servers": [], "total": 0, "error": str(exc)}


@app.post("/mcp/connect")
async def connect_mcp_server(req: MCPConnectRequest) -> dict:
    """Connect to an MCP server using a command string.

    The user provides a command (e.g. "npx @modelcontextprotocol/server-filesystem /path")
    and the backend starts the server and discovers available tools.
    """
    try:
        if _tool_registry is None:
            raise HTTPException(status_code=503, detail="Tool registry not initialised")
        ...
        # For stdio-based MCP servers, we use the command + args
        server_name = req.command.split("/")[-1] if "/" in req.command else req.command
        server_url = f"stdio://{req.command}"

        # Try connecting via the MCP client
...
@app.post("/mcp/connect-preset")
async def connect_mcp_preset(preset_name: str = "github") -> dict:
    """Connect to a known MCP server by preset name.

    Presets are pre-configured servers (github, stripe, figma, etc.)
    that resolve their auth tokens from environment variables.
    """
    try:
        if _tool_registry is None:
            raise HTTPException(status_code=503, detail="Tool registry not initialised")
        ...
        manager = _tool_registry._get_mcp_manager()
        conn = await manager.auto_connect_preset(preset_name)
        ...
        logger.error("MCP preset connect failed: %s", exc)
        return {"connected": False, "error": str(exc)}


@app.get("/mcp/presets")
async def list_mcp_presets() -> dict:
    ...
    try:
        ...
        logger.error("Failed to list MCP presets: %s", exc)
        return {"presets": {}, "total": 0, "error": str(exc)}
...
@app.post("/mcp/disconnect/{server_name}")
